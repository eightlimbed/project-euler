...
(2..1_000_000).each do |n|
  # change number to string, split, then map them to integers
  split_num = n.to_s.split(//).map(&:to_i)

  powered_nums = split_num.map { |n| n ** 5 }
  sum = powered_nums.inject(&:+)

  special_numbers << sum if sum == n
end

p special_numbers.inject(&:+)
# => 443839
