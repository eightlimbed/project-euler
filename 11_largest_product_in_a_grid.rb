...
# Change the grid to integers and break it up into rows of 20
grid = GRID.split(" ").map(&:to_i).each_slice(20).to_a

# Make grid in columns of 20
def columns(grid)
  columns = []
  (0...20).each do |x|
    (0...20).each do |y|
...
def up_down(grid)
  greatest = 0
  (0...20).each do |x|
    column = columns(grid)[x]
    i = 0
    while i < column.size
      product = column[i..i+3].inject(&:*)
      greatest = product if product > greatest
      i += 1
    end
  end
...
def diag(grid)
  greatest = 0
  (0..16).each do |x| # don't go out of bounds
    (0..16).each do |y|
...
# Check direction: reverse diagonal
def rev_diag(grid)
  greatest = 0
  (0..16).each do |x|
    (0..16).each do |y|
